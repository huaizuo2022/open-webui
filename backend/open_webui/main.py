...
def detect_company_resource_route(user_message: str) -> Optional[dict]:
    ...
    if re.search(r'https?://[^\s]*jira\.qima-inc\.com/', text) or re.search(r'\bonline-\d+\b', user_message, re.IGNORECASE):
        return {'skill_id': 'zan-jira', 'request': user_message}

    if 'qima.feishu.cn/wiki/' in text:
        return {'skill_id': 'feishu-wiki-skill', 'request': user_message}

    if re.search(r'https?://[^\s]*\.(qima-inc\.com|youzan\.com)', text):
        return None
